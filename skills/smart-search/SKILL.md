---
name: smart-search
description: 基於 opencli 命令的智慧搜尋路由器。當使用者想要搜尋、查詢、查找或研究資訊時，尤其是涉及指定網站、社交媒體、技術資料、新聞、購物、旅遊、求職、金融或中文內容時，務必使用此 skill
triggers:
  - "搜尋"
  - "查詢"
  - "查找"
...
### Gate 分类器：CoT vs 工具路由

在呼叫工具前，先判断问题类型：

**使用 CoT 直接回答（不呼叫工具）**：
- 定义/概念解释（「什么是 X」、「X 的意思」）→ 但不含「外部文件/官方文件」查询
- 通用知识/常识问题 → 但不含「2026/最新/今年」时间敏感问题
- 数学/逻辑推理问题
- 代码编写/调试（无需外部文件）
- 意见/建议/脑力激荡 → 但不含「技术选型/产品比较/价格规格」查询
- 翻译/改写/摘要（用户已提供内容）
...
- 特定网站内容（「在 PTT 找 X」）
- 学术/技术文件查询
- **新增**：外部文件/官方文件查询（「找 X 的 docs」、「官方文件」）
- **新增**：技术选型/产品比较（「X vs Y」、「哪个好」涉及规格/价格）
...
若 Gate 分类器判断为 CoT 路径，回答开头简短说明：

```md
**搜寻策略**：本问题属于 [类型]，使用 CoT 直接回答，未呼叫工具。
```

类型范例：定义解释、通用知识、推理问题、程式编写、意见建议
...
當查詢明確包含台灣地名、法規、貨幣（NTD）、本地服務（如「健保」、「勞基法」、「台股」、「高雄」等）時：

1. **AI 源優先級調整**
   - 優先使用 `gemini`（全球網頁覆蓋）或 `grok`（X 上有大量台灣討論）
   - **降低 `doubao` 優先級**（其訓練資料偏向中國大陸語境）

2. **禁止使用的大陸源**
   - 不要將 `weibo`、`zhihu`、`tieba` 作為台灣查詢的補充源

3. **需要原始討論時的 fallback**
   - 優先嘗試 `dcard`、`ptt`（adapter 存在時）
   - 否則使用 `opencli google` + `site:` 語法：
     ```
...
4. **反爬站點的最終 fallback**
   - 若 Google 結果不足，回退到 `WebSearch` 或 `opencli browser`
   - Dcard 等反爬嚴格站點，使用 `opencli browser` 驅動真實 Chrome 視窗

詳細台灣站點清單與 fallback 路徑，請參考 `references/sources-taiwan.md`。
